...
func parseToWindowsRouteStruct(output []byte, arg string) (windowsRouteStruct, error) {
	...
	for idx, line := range lines {
		...
		if sep == 3 {
			...
			if arg != "" {
				ipaddr := strings.TrimSpace(fields[3])
...
func parseToLinuxRouteStruct(output []byte, ifaceName string) (linuxRouteStruct, error) {
	...
	for scanner.Scan() {
		...
		if ifaceName != "" && ifaceName != tokens[0] {
			continue
		}
...
func parseWindowsGatewayIP(output []byte, arg string) (net.IP, error) {
	parsedOutput, err := parseToWindowsRouteStruct(output, arg)
	if err != nil {
...
func parseWindowsInterfaceIP(output []byte, arg string) (net.IP, error) {
	parsedOutput, err := parseToWindowsRouteStruct(output, arg)
	if err != nil {
		return nil, err
	}
...
func parseLinuxGatewayIP(output []byte, iface string) (net.IP, error) {

	parsedStruct, err := parseToLinuxRouteStruct(output, iface)
	if err != nil {
		return nil, err
	}
...
func parseLinuxInterfaceIP(output []byte, arg string) (net.IP, error) {
	parsedStruct, err := parseToLinuxRouteStruct(output, arg)
	if err != nil {
		return nil, err
	}
...
func parseDarwinRouteGet(output []byte, arg string) (net.IP, error) {
	...
	for _, line := range lines {
		...
		if len(fields) >= 2 && fields[0] == "gateway:" {
			...
			if ip != nil {
				if arg != "" {
					ips1 := strings.Split(ip.String(), ".")
					ips2 := strings.Split(arg, ".")
